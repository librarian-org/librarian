...
export class GenericSoftDeleteListener extends ListenerBase {
  static listenerName = 'softDelete';
  ...
  public async handler(event: IpcMainEvent, content: Event[]): Promise<void> {
    try {
      ...
      const repository = this.repositoryFactory.make('GenericSoftDelete', entity);
